...
const OrangeBlueCropGrowthBlockComponent = {
    onRandomTick({ block }) {
        const growthChance = 1 / 3;
        if (Math.random() > growthChance) return;

        const growth = block.permutation.getState("palamod:growth");
        block.setPermutation(
            block.permutation.withState("palamod:growth", growth + 1)
        );
    },
    onPlayerInteract({ block, dimension, player }) {
        ...
        if (player.getGameMode() === GameMode.creative) {
            // Grow crop fully
            block.setPermutation(block.permutation.withState("palamod:growth", 7));
        } else {
            let growth = block.permutation.getState("palamod:growth");

            // Add random amount of growth
            growth += randomInt(1, maxGrowth - growth);
            block.setPermutation(
                block.permutation.withState("palamod:growth", growth)
            );
...
world.beforeEvents.worldInitialize.subscribe(({ blockComponentRegistry }) => {
    blockComponentRegistry.registerCustomComponent(
        "palamod:orange_blue_crop_growth",
        OrangeBlueCropGrowthBlockComponent
    );
});
